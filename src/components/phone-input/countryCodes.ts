...
export const PHONE_VALIDATION: Record<string, z.ZodSchema> = {
  US: z.string().length(10, 'Phone number must be 10 digits'),
  CA: z.string().length(10, 'Phone number must be 10 digits'),
  IN: z.string().length(10, 'Phone number must be 10 digits'),
};

export const DEFAULT_PHONE_SCHEMA = z
  .string()
  .min(5, 'Phone number must be 5-15 digits')
  .max(15, 'Phone number must be 5-15 digits');
