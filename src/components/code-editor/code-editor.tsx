...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  const handleEditorDidMount: Parameters<typeof Editor>[0]['onMount'] = (
    editor,
    monaco
  ) => {
  ...
  return (
    <div className='~overflow-hidden ~rounded-lg ~border ~border-gray-300 ~shadow-md'>
      {/* Menu Bar */}
      <div className='~flex ~items-center ~justify-between ~bg-gray-100 ~px-8 ~py-2'>
        <p>
          <b>Type: </b>
          <span className='~capitalize'>{passedLanguage}</span>
        </p>
      ...
      {statusMessage && (
        <div className='~mb-2 ~px-4 ~py-1 ~text-right ~text-green-500'>
          {statusMessage}
        </div>
      )}
      ...
      {/* Helper Menu */}
      {showHelper && (
        <div className='~mt-2 ~rounded-md ~bg-gray-200 ~p-4 ~shadow-md'>
          <h3 className='~mb-2 ~font-bold'>Code Editor Help</h3>
          <ul className='~list-disc ~pl-5'>
            <li>
              <span className='~flex ~items-center'>
                <FaSun /> / <FaMoon />: Toggle between dark and light mode.
              </span>
            </li>
            <li>
              <span className='~flex ~items-center'>
                <FaBrush />: Format the code using Prettier.
              </span>
            </li>
            <li>
              <span className='~flex ~items-center'>
                <FaCopy />: Copy the current code to the clipboard.
              </span>
            </li>
            <li>
              <strong>Font Size:</strong> Adjust the font size using the input
              box.
            </li>
            <li>
              <strong>Token Suggestions:</strong> Type `@@` to see the Tokens
              suggestions.
            </li>
            <li>
              <strong>Shortcuts:</strong> Use <code>Ctrl+Space</code> to trigger
